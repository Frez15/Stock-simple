...
async function extractSessionCookie(loginResp) {
  ...
  try {
    ...
    if (inlineCookie) {
      if (inlineCookie.includes('=')) {
        cookieSet.add(inlineCookie.split(';')[0]);
      } else {
        cookieSet.add(`JSESSIONID=${inlineCookie}`);
      }
    }
  } catch (_) {
    // ignore JSON parsing errors; rely on headers instead
  }

  const raw =
    (typeof loginResp.headers.raw === 'function'
      ? loginResp.headers.raw()['set-cookie']
      : null) ||
    loginResp.headers.get('set-cookie');

  const cookieEntries = Array.isArray(raw)
    ? raw
    : typeof raw === 'string'
    ? raw.split(/,(?=[^ ;]+=)/)
    : [];
  ...
  for (const entry of cookieEntries) {
    ...
    if (!pair) continue;
    cookieSet.add(pair.trim());
  }

  if (!cookieSet.size) return undefined;

  return Array.from(cookieSet).join('; ');
}

async function requestPriceList(sessionCookie, baseUrl, params) {
...
async function handler(req, res) {
  ...
  try {
    ...
    const performRequest = async (params) => {
      try {
        const response = await requestPriceList(
          sessionCookie,
          CHESS_API_BASE,
          params
        );
