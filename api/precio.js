...
async function handler(req, res) {
  ...
  try {
    ...
    for (const candidate of requestedIds) {
      const priceResp = await requestPriceList(sessionId, CHESS_API_BASE, {
        Fecha: hoy,
        Lista: lista,
        CodArt: candidate,
      });
      const payload = await priceResp.json();
      lastPayload = payload;
    ...
    const fallbackResp = await requestPriceList(sessionId, CHESS_API_BASE, {
      Fecha: hoy,
      Lista: lista,
    });
    const fallbackPayload = await fallbackResp.json();
    lastPayload = fallbackPayload;
    const fallbackEntries = collectEntries(fallbackPayload);
