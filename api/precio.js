...
async function handler(req, res) {
  ...
  try {
    ...
    const pickSessionFromObject = (obj) => {
      if (!obj || typeof obj !== 'object') return null;
      for (const [key, value] of Object.entries(obj)) {
        if (typeof value !== 'string') continue;
        if (/session/i.test(key) && value.trim()) {
          return value.trim();
        }
        if ((/token/i.test(key) || /cookie/i.test(key)) && /session/i.test(value)) {
          return value.trim();
        }
      }
      return null;
    ...
    let sessionId = pickSessionFromObject(loginData);

    if (!sessionId && typeof loginText === 'string') {
      const match = loginText.match(/JSESSIONID=[^"'\s;]+/i);
      if (match) {
        sessionId = match[0];
      }
    }
